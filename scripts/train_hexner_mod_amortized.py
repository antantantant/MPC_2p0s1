# scripts/train_hexner_mod_amortized.py
...
def _maybe_load_resume(
    ...
    print(
        f"[train_hexner_mod_amortized] Resumed from '{resume_from}' "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train an amortized alpha model f(x0, p0) -> logits for Hexner-mod. "
    ...
    parser.add_argument("--run-dir", type=str, default="runs/hexner_mod_amortized")
...
def main() -> None:
    ...
    if int(args.num_val_states) > 0:
        ...
        print(
            f"[train_hexner_mod_amortized] Validation set: {int(args.num_val_states)} "
    ...
    for step in tqdm(range(start_step, int(args.max_iters))):
        ...
        if step % int(args.print_every) == 0 or step == int(args.max_iters) - 1:
            print(
                f"[train_hexner_mod_amortized] step={step:06d} "
        ...
        if step % int(args.checkpoint_every) == 0 or step == int(args.max_iters) - 1:
            ...
            if val_loss_val is None:
                print(f"[train_hexner_mod_amortized] Saved checkpoint to '{ckpt}'")
            else:
                print(
                    f"[train_hexner_mod_amortized] Saved checkpoint to '{ckpt}' "
    ...
    elapsed = t1 - t0
    print(
        f"[train_hexner_mod_amortized] Total training time: {elapsed:.2f} s "
    ...
    print(f"[train_hexner_mod_amortized] Saved metrics to '{metrics_path}'")
