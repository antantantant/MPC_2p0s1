# outer_opt/optimizer_loop_adjoint.py
...
from .objective_primal_adjoint import primal_objective_adjoint


def run_outer_optimization_adjoint(
    game: BaseLQGame,
    game_cfg: GameConfig,
    ...
    using the experimental manual-adjoint objective.

    Logging format intentionally matches optimizer_loop.run_outer_optimization.
    """
    ...
    for step in tqdm(range(start_step, train_cfg.max_iters)):
        ...
        def closure_lbfgs():
            ...
            with torch.cuda.amp.autocast(enabled=False):
                ...
                loss_val = primal_objective_adjoint(
                    game=game,
                    alpha_module=alpha_module,
                    indexer=indexer,
                    x0=x0,
