#!/usr/bin/env python3
"""Test small-angle nonlinear SQP with LQ warm-start."""
...
def main():
    print("=" * 80)
    print("Testing Small-Angle Quadrotor SQP with LQ Warm-Start")
    print("=" * 80)
    ...
    cfg = GameConfig(
        ...
        small_angle_approx=True,  # <-- Polynomial trig approx
        device="cpu",
        dtype=torch.float64,
    )
    ...
    # Run SQP with LQ-trained alpha + small-angle dynamics
    print("\n" + "=" * 80)
    print("Running SQP with LQ-trained alpha + small-angle dynamics")
    print("=" * 80)
    ...
    sqp_result = sqp_tree_layer(
        ...
        p0=prior,
        num_sqp_iters=20,
        step_size=0.3,        # More conservative
        riccati_reg=1e-1,     # Higher regularization
        verbose=True,
        early_stop=True,
        line_search=True,
